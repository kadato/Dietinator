...
const meetsExpo =
  major > 20 || (major === 20 && minor > 19) || (major === 20 && minor === 19 && patch >= 4)

if (!meetsExpo) {
  const fakeNodePkg = process.execPath.includes(`${path.sep}node_modules${path.sep}node${path.sep}`)
  ...
  if (fakeNodePkg) {
    console.error("")
    console.error('The npm package named "node" in your home folder is shadowing real Node.js.')
