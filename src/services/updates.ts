...
export function isNewerThan(tag: string, installed: string): boolean {
  ...
  return (
    ...
    (release[0] === current[0] &&
      (release[1] > current[1] ||
        (release[1] === current[1] && release[2] > current[2])))
  );
}
...
export function getApkAsset(
  release: GitHubRelease,
): GitHubReleaseAsset | null {
  return (
    release.assets.find(
      (asset) => asset.name.toLowerCase() === ANDROID_APK_ASSET.toLowerCase(),
    ) ?? null
  );
}
...
export function parseRelease(payload: unknown): GitHubRelease {
  ...
  return {
    ...
    publishedAt:
      typeof root.published_at === 'string' ? root.published_at : null,
