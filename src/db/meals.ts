...
export async function getMeals(): Promise<Meal[]> {
  ...
  const items = await db.getAllAsync<MealItemRow>("SELECT * FROM meal_items ORDER BY position ASC")
...
export async function getMealById(id: string): Promise<Meal | null> {
  const db = await getDatabase()
  const meal = await db.getFirstAsync<MealRow>("SELECT * FROM meals WHERE id = ?", id)
  if (!meal) return null
